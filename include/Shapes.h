...
public:
    ...
    Rectangle(double ulcx, double ulcy, double length, double width, string name = "r"):
...
public:
    ...
    Circle(double centerX, double centerY, double radius,string name = "c"):
...
public:
    ...
    Triangle(vertex vertex_A, vertex vertex_B, vertex vertex_C, string name = "t"):
        Shape(name), v1(vertex_A), v2(vertex_B), v3(vertex_C) {

            if(isTriangle(vertex_A, vertex_B, vertex_C) == false)
                throw string("It's not a triangle.");
